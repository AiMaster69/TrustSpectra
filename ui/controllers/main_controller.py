...
class MainController:
    def __init__(self, settings_service: SettingsService):
        ...
        logger.info(f"Главный контроллер инициализирован с порогом {threshold}, чанки: {chunk_duration}s")
    ...
    def apply_settings(self, settings: Dict[str, Any]) -> None:
        ...
        if performance := settings.get("performance"):
            ...
            logger.info(f"Настройки производительности сохранены, применятся при перезапуске: {performance}")
    ...
    def analyze_file(self, file_path: str, progress_callback=None) -> AnalysisResult:
        ...
        try:
            ...
            result = self.analyzer.analyze_audio(file_path, progress_callback=progress_callback)
            ...
            analysis_result = AnalysisResult(
                ...
                parameters={
                    'threshold': self.get_analysis_settings().get('confidence_threshold', 0.9),
            ...
            if self.current_audio_file and str(self.current_audio_file.path) == file_path:
    ...
    def analyze_current_file(self, progress_callback=None) -> AnalysisResult:
        ...
        file_path = str(getattr(self.current_audio_file, 'path', self.current_audio_file))
    ...
    def set_performance_settings(self, settings: Dict[str, Any]):
        ...
        logger.info(f"Настройки производительности обновлены и применятся при следующем запуске: {settings}")
