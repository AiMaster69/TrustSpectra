...
class FileItemWidget(QFrame):
    ...
    def __init__(self, file_path: str, file_size: int, duration: int = None, parent=None):
    ...
    def _setup_ui(self):
        ...
        if self.duration is not None and self.duration > 0:
            self.duration_label = QLabel(f"{self.duration // 60}:{self.duration % 60:02d}")
        else:
...
class FileListWidget(QWidget):
    ...
    def _update_height(self):
        ...
        if not self.files:
            ...
            total_height = (rows * 70) + max(0, rows - 1) * SIZES['spacing'] + 2 * SIZES['spacing']
