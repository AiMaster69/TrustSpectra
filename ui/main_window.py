...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QHBoxLayout, QApplication, QDialog
)
from PyQt6.QtCore import Qt, QPoint, QEvent, QThread, pyqtSignal
from PyQt6.QtGui import QGuiApplication, QMouseEvent, QIcon, QScreen
...
from ui.styles.themes import COLORS, SIZES, update_theme_colors, save_theme
from ui.styles.style_factory import sf
from ui.styles.style_factory import refresh_all_styles
from ui.widgets.themed_dialog import ThemedMessageBox, AIWarningDialog
    ...
    def run(self) -> None:
        for file_path in self.files:
            ...
            try:
                ...
                # Теперь вызываем анализ через контроллер, который правильно преобразует результат
                result = self.controller.analyze_file(file_path, progress_callback=progress_cb)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.setMinimumSize(SIZES["content_min_width"] + SIZES["content_margin"] * 2, 600)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        self.setWindowFlags(Qt.WindowType.FramelessWindowHint | Qt.WindowType.WindowMaximizeButtonHint)
        ...
        self._analysis_queue: list[str] = []  # Очередь для файлов, ожидающих анализа
    ...
    def _setup_app_icon(self) -> None:
        icon_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "resources", "icons", "icon.svg"))
    ...
    def eventFilter(self, obj: object, event: QEvent) -> bool:
        if event.type() == QEvent.Type.MouseMove and not self._is_window_maximized():
            if isinstance(event, QMouseEvent):
                ...
                self._update_cursor(pos) if self.rect().contains(pos) else self.unsetCursor()

        elif self.settings_widget.isVisible():
            if event.type() == QEvent.Type.WindowDeactivate:
                self.settings_widget.instant_hide_settings()
            elif event.type() == QEvent.Type.MouseButtonPress and isinstance(obj, QWidget):
                if obj not in (self.settings_widget, self.settings_button) and not self.settings_widget.isAncestorOf(obj):
                    self.settings_widget.instant_hide_settings()
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if not self._is_window_maximized() and (edge := self._get_edge_at(pos)):
                edge_map = {"left": Qt.Edge.LeftEdge, "right": Qt.Edge.RightEdge, "bottom": Qt.Edge.BottomEdge,
                            "bottom-left": Qt.Edge.BottomEdge | Qt.Edge.LeftEdge, "bottom-right": Qt.Edge.BottomEdge | Qt.Edge.RightEdge}
                if self.windowHandle() and self.windowHandle().startSystemResize(edge_map[edge]):
                    return event.accept()

            if self.title_bar.geometry().contains(pos):
                is_on_btn = any(btn.geometry().contains(self.title_bar.mapFrom(self, pos)) for btn in self.title_bar.buttons.values())
                if pos.x() <= self.title_bar.geometry().right() - self.TITLE_BAR_RESIZE_THRESHOLD and not is_on_btn:
    ...
    def _update_cursor(self, pos: QPoint) -> None:
        cursors = {"left": Qt.CursorShape.SizeHorCursor, "right": Qt.CursorShape.SizeHorCursor,
                   "bottom": Qt.CursorShape.SizeVerCursor, "bottom-left": Qt.CursorShape.SizeBDiagCursor,
    ...
    def _on_worker_progress(self, file_path: str, percent: int, remaining: float) -> None:
    ...
    def _show_ai_warning_if_needed(self) -> None:
        if not self._ai_warning_shown_this_session and self.settings_service.get_setting("ui.show_ai_warning", True):
    ...
    def _on_worker_all_finished(self) -> None:
        # Если в очереди еще есть файлы, запускаем следующую партию
    ...
    def load_audio(self) -> None:
        try:
            ...
            file_paths, _ = QFileDialog.getOpenFileNames(self, get_text("ui.dialogs.select_audio"), "", filters)
    ...
    def analyze_audio(self) -> None:
        if not self.file_list.files:
            return ThemedMessageBox.information(self, get_text("ui.messages.info"), get_text("ui.messages.load_files_first"))
        ...
        # Исключаем уже проанализированные, уже стоящие в очереди и уже анализируемые файлы
        to_analyze = [f for f in self.file_list.files if f not in analyzed and f not in queued and f not in processing]

        if not to_analyze:
            return ThemedMessageBox.information(self, get_text("ui.messages.info"), get_text("ui.messages.all_files_analyzed"))

        self.timeline.start_analysis(to_analyze)

        if self.is_analyzing():
            # Если анализ уже идет, просто добавляем новые файлы в очередь
            self._analysis_queue.extend(to_analyze)
        else:
            # Если анализ не идет, запускаем поток
            self._taskbar.start(len(to_analyze))
            self._start_worker(to_analyze)

    def _start_worker(self, files: list[str]) -> None:
        """Вспомогательный метод для инициализации и запуска воркера."""
    ...
    def toggle_settings(self) -> None:
        if getattr(self.settings_widget, "_is_animating", False): return
        
        if self.settings_widget.isVisible() and self.settings_widget.width() > 0:
            self.settings_widget.hide_settings()
        else:
            global_pos = self.settings_button.mapToGlobal(self.settings_button.rect().bottomLeft())
            local_pos = (self.settings_widget.parent() or self).mapFromGlobal(global_pos)
            w = min(400, self.width() // 3)
            self.settings_widget.adjustSize()
            h = self.settings_widget.sizeHint().height()
            self.settings_widget.setGeometry(local_pos.x(), max(0, min(local_pos.y(), self.height() - h)), w, h)
            self.settings_widget.show_settings(self.width())
            self.settings_widget.raise_()
            self.settings_button.raise_()
            
        self.settings_button.setChecked(self.settings_widget.isVisible() and self.settings_widget.width() > 0)
    ...
    def _resolve_actual_theme(self, theme_name: str) -> str:
        ...
        is_dark = self.is_system_theme_dark()
    ...
    def on_settings_changed(self, settings: dict) -> None:
        ...
        if lang := ui_settings.get("language"):
            ...
            for w in (self.timeline, self.settings_widget):
                if hasattr(w, "update_texts"): w.update_texts()
    ...
    def update_styles(self) -> None:
        ...
        for w in (self.title_bar, self.player, self.timeline, self.file_list, self.settings_widget):
            if hasattr(w, "update_styles"): w.update_styles()

    def closeEvent(self, event) -> None:
        # Проверяем и активный воркер, и очередь ожидания
        if self.is_analyzing() or self._analysis_queue:
            res = ThemedMessageBox.confirm(self, title=get_text("ui.messages.analysis_in_progress_title"),
                                           message=get_text("ui.messages.analysis_in_progress_message"),
                                           icon_type="warning", cancel_text=get_text("ui.buttons.cancel"),
                                           confirm_text=get_text("ui.buttons.close_anyway"))
            if res == QDialog.DialogCode.Rejected:
                return event.ignore()
            if self._worker:
                self._worker.stop()
            self._analysis_queue.clear()  # Очищаем очередь при закрытии
