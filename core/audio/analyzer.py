...
try:
    ...
    logger.critical(f"Критическая ошибка: не удалось импортировать onnxruntime. Детали: {e}")
    ort = None

try:
    import librosa
except ImportError as e:
    logger.critical(f"Критическая ошибка: не удалось импортировать librosa (возможно, отсутствует scipy или soundfile). Детали: {e}")
    librosa = None
...
def create_analyzer_adapter(threshold=0.9, num_threads=None, chunk_duration=None, use_chunked_loading=True):
    ...
    if analyzer.is_ready:
        ...
        logger.error("Анализатор инициализирован, но модель ONNX не найдена или не загружена.")
        raise RuntimeError("Не удалось загрузить ONNX модель. Убедитесь, что файл model.onnx существует.")
    ...
    def update_parameters(self, **kwargs):
        ...
        if 'confidence_threshold' in kwargs or 'threshold' in kwargs:
            ...
            if t is not None:
                ...
                self.confidence_thresholds = {
                    cls: val for cls, val in self.confidence_thresholds.items()
                    if abs(val - self.confidence_threshold) > 1e-9
                }
...
class TrustSpectra(BaseAnalyzer):
    ...
    def __init__(self, model_path=None, device=None, confidence_threshold=0.6,
                 num_threads=None, chunk_duration=60.0, use_chunked_loading=True):
        ...
        self._mel_basis = librosa.filters.mel(
            ...
            fmin=self.F_MIN, fmax=self.F_MAX, htk=True, norm=None
        )
            ...
            try:
                ...
                if 'class_names' in metadata:
                    ...
                    if 'optimal_thresholds' in metadata:
                        self.confidence_thresholds = {}
                        for cls, thr in zip(self.CLASSES, metadata['optimal_thresholds']):
                            if thr > 1.0:
                                thr /= 100.0
    ...
    def analyze_audio(self, file_path: str, progress_callback=None) -> Dict:
        ...
        try:
            ...
            use_chunked = self.use_chunked_loading and (duration > self.MAX_FILE_DURATION)
            if use_chunked:
    ...
    def _extract_mel(self, batch_audio: List[np.ndarray]) -> np.ndarray:
        ...
        for audio in batch_np:
            S = np.abs(librosa.stft(audio, n_fft=self.N_FFT, hop_length=self.HOP_LEN))**2
    ...
    def _run_inference(self, batch_audio: List[np.ndarray], base_times: List[float], duration: float) -> List[Dict]:
        ...
        for i, prob_arr in enumerate(probs):
            for cls_idx, prob in enumerate(prob_arr):
                ...
                target_thr = self.confidence_thresholds.get(cls_name, self.confidence_threshold)
                if prob >= target_thr:
                    results.append({
    ...
    def _analyze_standard(self, file_path: str, duration: float, progress_cb) -> Dict:
        ...
        for b_start in range(0, total_wins, 16):
            ...
            for idx in range(b_start, b_end):
                start = idx * step_smp
                b_audio.append(self._fast_normalize_window(y[start:start + self.win_smp]))
        ...
        if progress_cb:
            progress_cb(100)
        return self._format_results(file_path, duration, self._merge_segments(results), time.time() - start_t)
    ...
    def _analyze_chunked(self, file_path: str, duration: float, progress_cb) -> Dict:
        ...
        for chunk_idx in range(num_chunks):
            ...
            if chunk_dur <= 0:
                break
            try:
                y, sr = librosa.load(file_path, sr=self.SR, mono=True, offset=offset, duration=chunk_dur)
            except Exception as e:
        ...
        if progress_cb:
            progress_cb(100)
        return self._format_results(file_path, duration, self._merge_segments(results), time.time() - start_t, chunked=True)
    ...
    def _format_results(self, file_path, duration, merged, proc_time, chunked=False) -> Dict:
        scores = {c: {'probability': 0.0, 'detected': False, 'confidence': 0.0} for c in self.CLASSES}
        for r in merged:
            if r['confidence'] > scores[r['label']]['probability']:
                scores[r['label']].update({'probability': r['confidence'], 'detected': True, 'confidence': r['confidence']})
        return {
            'file_path': file_path, 'segments': merged, 'detected_classes': [r['label'] for r in merged],
            'class_scores': scores, 'has_detections': bool(merged), 'processing_time': proc_time,
            'file_duration': duration, 'model_used': self.model_path, 'chunked_processing': chunked
        }